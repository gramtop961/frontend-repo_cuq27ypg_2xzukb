...
function App() {
  ...
  useEffect(() => {
    ...
    // ===== Moved inline script logic into effect so it executes correctly =====
    // Sample catalog
    const CATALOG = [
    ...
    const applyTheme = () => {
      ...
      if (themeBtn) themeBtn.innerHTML = t==='light' ? '<i class="fa-solid fa-sun"></i>' : '<i class="fa-solid fa-moon"></i>'
    }
    ...
    if(track){
      ...
      // Auto slide scroll
      const startAuto = ()=> autoScroll = setInterval(()=>{ track.scrollBy({left:260, behavior:'smooth'}); if(track.scrollLeft + track.clientWidth >= track.scrollWidth-10){ track.scrollTo({left:0, behavior:'smooth'});} }, 2500);
    ...
    const showHome = (show) => { const hw = document.getElementById('homeWrap'); if(hw) hw.style.display = show ? '' : 'none'; }
    ...
    const shopView = () => {
      ...
      const apply = () => {
        let list = [...CATALOG];
        const cat = document.getElementById('fCat').value; const brand=document.getElementById('fBrand').value; const color=document.getElementById('fColor').value; const sort=document.getElementById('fSort').value; const s=document.getElementById('fSearch').value.toLowerCase() || q;
    ...
    const productView = () => {
      ...
      imgs.forEach((src,i)=>{ const im = document.createElement('img'); im.src = src; if(i===0) im.classList.add('active'); im.addEventListener('click', ()=>{ document.getElementById('mainImg').src = src; $$('#thumbs img').forEach(t=>t.classList.remove('active')); im.classList.add('active'); }); thumbs.appendChild(im); });
      let sel = {size: (p.sizes||[])[0], color: (p.colors||[])[0]};
      document.getElementById('sizes')?.addEventListener('click', e=>{ const o=e.target.closest('[data-size]'); if(o){ sel.size=o.dataset.size; $$('#sizes .option').forEach(x=>x.classList.toggle('active', x===o)); }});
      document.getElementById('colors')?.addEventListener('click', e=>{ const o=e.target.closest('[data-color]'); if(o){ sel.color=o.dataset.color; $$('#colors .option').forEach(x=>x.classList.toggle('active', x===o)); }});
      document.getElementById('addDetail').addEventListener('click', ()=>{ addToCart(p.id, 1, sel); alert('Added to cart'); });
      document.getElementById('wishDetail').addEventListener('click', ()=>{ toggleWish(p.id); alert('Updated wishlist'); });
    ...
    const cartView = () => {
      ...
      const onClick = (e)=>{
        ...
        if(inc){ const it = cart.find(x=>x.key===inc.dataset.inc); updateQty(it.key, it.qty+1); renderList(); }
        if(dec){ const it = cart.find(x=>x.key===dec.dataset.dec); updateQty(it.key, Math.max(1,it.qty-1)); renderList(); }
        if(rem){ removeFromCart(rem.dataset.rem); renderList(); }
      };
      V.addEventListener('click', onClick);
      renderList();
    };
    ...
    const contactView = () => {
      ...
      document.getElementById('contactForm').addEventListener('submit', (e)=>{ e.preventDefault(); alert('Message sent!'); location.hash='#'; })
    };
